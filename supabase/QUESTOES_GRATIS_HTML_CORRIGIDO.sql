...
create index if not exists free_quizzes_date_idx
  on public.free_quizzes(publish_date);
...
create policy "free quizzes public read"
on public.free_quizzes
for select
to anon, authenticated
using (is_published = true or public.is_admin());

create policy "free quizzes admin all"
on public.free_quizzes
for all
to authenticated
...
-- Bucket separado e público somente para os HTMLs gratuitos.
insert into storage.buckets (id, name, public)
values ('free-quizzes', 'free-quizzes', true)
...
create policy "free quizzes admin insert"
on storage.objects
for insert
to authenticated
with check (
  bucket_id = 'free-quizzes'
  and public.is_admin()
);

create policy "free quizzes admin update"
on storage.objects
for update
to authenticated
using (
...
create policy "free quizzes admin delete"
on storage.objects
for delete
to authenticated
using (
